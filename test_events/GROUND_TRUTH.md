...
- `PR-12 --[REFERENCES]--> N-main`   ← TC-01: "main.py" resolved to existing node
- `PR-12 --[REFERENCES]--> N-db`     ← TC-02: init_db() resolved to its owning file
...
### New edges (TC-03)
- `PR-15 --[REFERENCES]--> N-auth`   ← TC-03: "auth module" resolved to existing node
...
- N-models: re-embed summary (no longer "SQLAlchemy ORM model", now "PyMongo dataclass")
...
### New nodes
- None (pydantic is an external lib; if created, should be typed `ExternalDependency`, NOT matched to any existing node)
...
(+ optional N-pydantic as ExternalDependency)
...
### A query like "what is the tech stack?" should return ONLY the 5 active facts above.
