...
- - **No new node** is created for `main.py`.
  - - An edge is drawn: `PR-12 --[REFERENCES]--> N-main`.
    ...
    ### TC-02 — PR references a function by name; resolves to the function node

    **Triggered by**: E02 (PR-12)
    **Input text**: `"Moving the init_db() call to after register_blueprint fixes the crash."`

    **Expected graph behaviour**:
    - `init_db()` is a function defined in `src/db.py`.
    - - The pipeline resolves `"init_db()"` to the **function node** `init_db` within `db.py` — NOT to the file node N-db and NOT to a new floating node.
      - - An edge is drawn: `PR-12 --[REFERENCES]--> N-init_db` (where N-init_db is the function-level node inside N-db).
       
        - **Failure mode to catch**:
        - - (a) Pipeline resolves to the file node N-db instead of the function node — too coarse.
          - - (b) Pipeline creates a brand new node for `init_db` disconnected from N-db.
            - - (c) Pipeline misses the reference entirely.
             
              - **Resolution signal**: Function name lookup against ingested code symbols. The pipeline
              - must be able to resolve named functions/methods/classes to their own nodes, not just
              - to their containing file.
             
              - **Note from teammate**: Functions, methods, classes, and modules are first-class nodes.
              - Entity resolution must target the most specific matching node, not just the file.
                  ...
                  **Expected graph behaviour**:
                  ...
                  - - The pipeline creates **one** edge: `T-001 --[REFERENCES]--> N-models`.
                    - - It does **not** create two separate edges or two separate nodes.
                      ...
                      ### TC-ERR-01 — PR references a library that is NOT in the graph
                              ...
                              - ```
                                ...
                                **Before (active facts)**:
                                ...
                                - - F06: `ARCHITECTURE.md` states `"Auth uses JWT tokens (ADR-002)"` ✅ active
                                  ...
                                  - **After event E06**:
                                  - - F09: `auth.py` states `"Auth uses session-based auth (Flask-Session + Redis)"` ✅ active
                                    - - F03: ❌ invalidated
                                      - - F06: ❌ invalidated
                                        ...
                                        - ```
                                          ...
                                          **Failure mode to catch**: Only F03 is invalidated (because README is the more obvious
                                          source), while F06 quietly remains active on the ARCHITECTURE node.
                                            ...
                                            - - F01: ❌ invalidated
                                              ...
                                              - ```
                                                ...
                                                **Additional edge to verify**:
                                                ...
                                                -   `last_modified` metadata and re-embed their summaries to reflect that they no longer
                                                -     describe a relational schema.
                                               
                                                - **Failure mode to catch**: Fact-level invalidation works (F01 → stale), but node-level
                                                - metadata for N-models and N-db still says "SQLAlchemy / PostgreSQL", causing a semantic
                                                - mismatch between graph facts and node embeddings.
                                                            ...
                                                            - - F05: ❌ invalidated
                                                              ...
                                                              - ```
                                                                ...
                                                                **Additional check — new nodes**:
                                                                Two new files are created in this PR: `Dockerfile` and `cloudbuild.yaml`. Verify:
                                                                - N-dockerfile and N-cloudbuild are **created** (not matched to existing nodes).
                                                                - - Edges: `PR-34 --[ADDS]--> N-dockerfile` and `PR-34 --[ADDS]--> N-cloudbuild`.
                                                                  ...
                                                                  |------------|-----------------------|--------|--------------------------------------------------------------|-------------------------------------------|
                                                                  | TC-01      | Entity Resolution     | E02    | Filename in PR text → existing file node                     | Duplicate node created for `main.py`      |
                                                                  | TC-02      | Entity Resolution     | E02    | Function name → **function node** (not file node)            | Resolved to file node N-db instead        |
                                                                  | TC-03      | Entity Resolution     | E03    | Module synonym → existing file node                          | Floating concept node created             |
